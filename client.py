            ...
            if message == "<<NICK>>":
                ...
                try:
                    ...
                    if data.get("type") == "message":
                        ...
                        console.print(
                            f"[{timestamp}] [{color}]{sender}[/{color}]: {content}"
                        )
            ...
            if text.lower() == "/quit":
                ...
                while i < len(args):
                    ...
                    if "=" in arg:
                        ...
                        value = args[i + 1]
                        i += 1
                    else:  # Flag without value? Ignore or store as empty?
                        key = arg
                        value = ""
                    ...
                    if key:
                        if value.lower() in ["", '""', "''"]:
                            if key in tts_settings:
                                del tts_settings[key]
                        else:
                            tts_settings[key] = value
                    i += 1

                console.print(
                    f"[bold green]TTS Settings updated:[/bold green] {tts_settings}"
                )
                continue
...
if __name__ == "__main__":
    ...
    receive_thread = threading.Thread(
        target=receive_messages, args=(client, speak_enabled)
    )
